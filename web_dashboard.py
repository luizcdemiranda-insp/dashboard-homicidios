...
if menu == "1. VISÃO GERAL (MACRO)":
    ...
    if sucesso_dados:
        ...
        if len(anos_selecionados) > 0:
            ...
            # --- ANÁLISE 4: DISTRIBUIÇÃO DAS ORCRIM (SELETOR MANUAL) ---
            st.markdown("### ⚖️ DISTRIBUIÇÃO DE ORCRIM")
            
            # Se o Python tiver 31 colunas ou mais, ele sugere a 30 (AE). Se não, sugere a primeira.
            indice_sugerido = 30 if len(df_filtrado.columns) > 30 else 0
            
            # O NOVO SELETOR MANUAL DE COLUNA
            coluna_orcrim_escolhida = st.selectbox(
                "⚠️ O sistema não conseguiu fixar a coluna AE automaticamente. Por favor, selecione abaixo qual é a coluna que contém os dados de ORCRIM:", 
                options=df_filtrado.columns.tolist(),
                index=indice_sugerido
            )

            if coluna_orcrim_escolhida and total_procedimentos > 0:
                orcrim_raw = df_filtrado[coluna_orcrim_escolhida].astype(str).str.upper()
                ...
                if len(df_orcrim) > 0:
                    ...
                    st.info("Nenhum dado válido de ORCRIM foi encontrado na coluna selecionada.")
            
            # --- MODO RAIO-X (DEPURAÇÃO) ---
            st.write("---")
            with st.expander("🕵️‍♂️ Modo Raio-X (Ver como o sistema enxerga a planilha)"):
                st.write("Verifique como as colunas da sua planilha estão sendo importadas. Procure pelos dados da ORCRIM e veja qual é o nome do cabeçalho que o Python deu a eles (pode estar como UNNAMED).")
                st.dataframe(df_filtrado.head(10)) # Mostra as 10 primeiras linhas
