...
# CSS PERSONALIZADO (Seu Design Incrível)
st.markdown("""
    <style>
    ...
    with col_meio:
        ...
        with aba_login:
            ...
            if st.button("Acessar Painel"):
                ...
                except: st.error("Erro ao conectar base de usuários. Verifique o compartilhamento da planilha.")

        with aba_cadastro:
            n_cad = st.text_input("Nome Completo")
            m_cad = st.text_input("Matrícula")
...
if not st.session_state.logado:
    ...
    menu = st.sidebar.radio("NAVEGAÇÃO", ["1. VISÃO GERAL", "2. CASOS POR ÁREA", "3. MOTIVAÇÃO / DELITO", "4. MODO ANALÍTICO", "5. ASSISTENTE IA", "⚙️ CONFIGURAÇÕES"] if st.session_state.user_nivel == "Master" else ["1. VISÃO GERAL", "2. CASOS POR ÁREA", "5. ASSISTENTE IA"])
    
    if st.sidebar.button("Sair"):
        st.session_state.logado = False
        st.rerun()

    # Sua Função de Gerar Dashboard (Preservada)
    def gerar_dashboard(df_filtrado):
        # ... (Todo o seu código de cards e gráficos que você enviou acima vai aqui dentro)
        COL_DIA = next((c for c in df_filtrado.columns if "DIA" in str(c) and "SEMANA" in str(c)), None)
        total_procedimentos = len(df_filtrado)
        c1, c2 = st.columns(2)
        with c1: st.markdown(f'<div style="background-color: #1E2130; padding: 25px; border-radius: 12px; border-top: 5px solid #ff4b4b; text-align: center;"><h3 style="color: #b0b4c4;">📊 TOTAL PROCEDIMENTOS</h3><h1 style="color: white;">{total_procedimentos}</h1></div>', unsafe_allow_html=True)
        # ... (restante dos gráficos do seu código mãe)

    # NAVEGAÇÃO
    if menu == "1. VISÃO GERAL":
        df = carregar_dados()
        # ... (Sua lógica de Filtro de Anos e chamada do gerar_dashboard)
    ...
    # ... (Demais blocos de menu conforme seu código mãe)
